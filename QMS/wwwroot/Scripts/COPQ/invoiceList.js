...
function renderInvoiceTable(response) {
    ...
    if (tableInvoice) {
        ...
        document.getElementById("invExportButton").addEventListener("click", function () {
            // Get only visible data from Tabulator (respects filters, sorting, pagination)
            var visibleData = tableInvoice.getData("active"); // "active" gets only visible/filtered rows

            // Get visible columns only
            var visibleColumns = tableInvoice.getColumns().filter(col => col.isVisible() && col.getField() !== "Action");
            ...
            if (filterStartDateInvoice && filterEndDateInvoice) {
                dateRangeText = `Date Range: ${moment(filterStartDateInvoice).format('DD-MMM-YYYY')} to ${moment(filterEndDateInvoice).format('DD-MMM-YYYY')}`;
            } else if (filterStartDateInvoice) {
                dateRangeText = `Date From: ${moment(filterStartDateInvoice).format('DD-MMM-YYYY')}`;
            } else if (filterEndDateInvoice) {
                dateRangeText = `Date To: ${moment(filterEndDateInvoice).format('DD-MMM-YYYY')}`;
            } else {
            ...
            var fileName = `InvoiceList_${moment().format('YYYYMMDD_HHmmss')}.xlsx`;
