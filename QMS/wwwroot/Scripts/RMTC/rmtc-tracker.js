...
var table = '';
let vendorOptions = {};
let filterStartRMTCDate = moment().startOf('month').format('YYYY-MM-DD');
let filterEndRMTCDate = moment().endOf('month').format('YYYY-MM-DD');

$(document).ready(function () {
    $('#dateRangeTextRMTC').text(
        moment(filterStartRMTCDate).format('MMMM D, YYYY') + ' - ' + moment(filterEndRMTCDate).format('MMMM D, YYYY')
    );

    // Initialize Litepicker for date range selection
    const picker = new Litepicker({
        element: document.getElementById('customDateTriggerRMTC'),
        singleMode: false,
        format: 'DD-MM-YYYY',
        numberOfMonths: 2,
        numberOfColumns: 2,
        ...
        setup: (picker) => {
            picker.on('selected', (start, end) => {
                filterStartRMTCDate = start.format('YYYY-MM-DD');
                filterEndRMTCDate = end.format('YYYY-MM-DD');
                $('#dateRangeTextRMTC').text(`${start.format('MMMM D, YYYY')} - ${end.format('MMMM D, YYYY')}`);
                loadData();
            });
            ...
            picker.on('clear', () => {
                filterStartRMTCDate = "";
                filterEndRMTCDate = "";
                $('#dateRangeTextRMTC').text("Select Date Range");
                loadData();
            });
        },
        ranges: {
    ...
    $('#customDateTriggerRMTC').on('click', function () {
        picker.show();
    });
...
function loadData() {
    ...
    $.ajax({
        ...
        $.ajax({
            url: '/RMTCTracker/GetAll',  // Change to your actual endpoint
            type: 'GET',
            dataType: 'json',
            data: {
                startDate: filterStartRMTCDate,
                endDate: filterEndRMTCDate
            }
        })
            .done(function (data) {
...
function OnTabGridLoad(response) {
    ...
    if (response.length > 0) {
        $.each(response, function (index, item) {
            tabledata.push({
                ...
                Housing_Body: item.housing_Body || "",
                Wires_Cable: item.wires_Cable || "",
                Diffuser_Lens: item.diffuser_Lens || "",
                Pcb: item.pcb || "",
                Connectors: item.connectors || "",
                Powder_Coat: item.powder_Coat || "",
                Led_LM80: item.led_LM80 || "",
                Led_Purchase_Proof: item.led_Purchase_Proof || "",
                Driver: item.driver || "",
                Pre_Treatment: item.pre_Treatment || "",
    ...
    const columns = [
        ...
        editableColumn("Date", "Date", "date", "center", "input", {}, {}, 120),
        ...
        editableColumn(
            ...
            { values: { Metal: "Metal", Plastic: "Plastic" } },  // headerFilterParams
            { values: { Metal: "Metal", Plastic: "Plastic" } },  // editorParams
            null
        ),
        ...
        editableColumn(
            ...
            "center",                      // hoz align
            "list",                        // header filter type
            { values: { "Wires": "Wires", "Cables": "Cables" } },  // headerFilterParams
            { values: { "Wires": "Wires", "Cables": "Cables" } },  // editorParams
            null
        ),
        ...
        editableColumn(
            ...
            "center",                      // hoz align
            "list",                        // header filter type
            { values: { "Diffuser": "Diffuser", "Lens": "Lens" } },  // headerFilterParams
        ...
        editableColumn(
            ...
            "center",                      // hoz align
            "list",                        // header filter type
            { values: { "LED LM80": "LED LM80", "Photo Biological": "Photo Biological" } },  // headerFilterParams
            { values: { "LED LM80": "LED LM80", "Photo Biological": "Photo Biological" } },  // editorParams
            null
        ),
        ...
        editableColumn(
            ...
            "center",                      // hoz align
            "list",                        // header filter type
            { values: { "SS": "SS", "MS": "MS" } },  // headerFilterParams
    ...
    if (table) {
        ...
        table = new Tabulator("#rmtc_table", {
            data: tabledata,
            layout: "fitDataFill",
            movableColumns: true,
            pagination: "local",
    ...
    document.getElementById("exportRMTCButton").addEventListener("click", async function () {
        // ===== 0) OPTIONS =====
        ...
        const TITLE_TEXT = "RM TC/INSPECTION REPORT TRACKER";
        ...
        const ws = wb.addWorksheet("RM TC/Inspection Report Tracker", {
        ...
        link.download = "RM TC/Inspection Report Tracker.xlsx";
...
$('#rmtc_table').on('change', '.pdi-upload', function () {
    ...
    $.ajax({
        url: "/RMTCTracker/UploadRMTCAttachment",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false
            ...
            buttons: [
                {
                    ...
                    click: function (notice, value) {
                        ...
                        $.ajax({
                            url: '/RMTCTracker/Delete',
                            type: 'POST',
                            data: { id: recid }
                        }).done(function (data) {
                            if (data && data.success === true) {
                                showSuccessNewAlert("RM TC Tracker Detail deleted successfully.");
...
function saveEditedRow(rowData) {
    ...
    const cleanedData = {
        ...
        Product_No: normalizeMultiToString(rowData.Product_No, ", "),
        ProdDesc: normalizeMultiToString(rowData.ProdDesc ?? rowData.ProdDesc, " | "),
        //Product_No: rowData.Product_No || null,
        //ProdDesc: rowData.ProdDesc || null,
        Date: toIsoDate(rowData.Date) || null,
        Housing_Body: rowData.Housing_Body || null,
        Wires_Cable: rowData.Wires_Cable || null,
        Diffuser_Lens: rowData.Diffuser_Lens || null,
        Pcb: rowData.Pcb || null,
        Connectors: rowData.Connectors || null,
        Powder_Coat: rowData.Powder_Coat || null,
        Led_LM80: rowData.Led_LM80 || null,
        Led_Purchase_Proof: rowData.Led_Purchase_Proof || null,
        Driver: rowData.Driver || null,
        Pre_Treatment: rowData.Pre_Treatment || null,
        Hardware: rowData.Hardware || null,
        Other_Critical_Items: rowData.Other_Critical_Items || null,
        Filename: rowData.Filename ? rowData.Attchment : null,
    ...
    const url = isNew ? "/RMTCTracker/Create" : "/RMTCTracker/Update";
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                ...
                showDangerAlert("RMTC Tracker Detail already exists.");
            }
            else {
                var errorMessg = "";
                if (response.errors) {
