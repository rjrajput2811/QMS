...
$(document).ready(function () {
    ...
    $('#dateRangeTPT').text(
        moment(filterStartTPTDate).format('MMMM D, YYYY') + ' - ' + moment(filterEndTPTDate).format('MMMM D, YYYY')
    );

    const picker = new Litepicker({
        element: document.getElementById('customDateTriggerTPT'),
        singleMode: false,
        format: 'DD-MM-YYYY',
        numberOfMonths: 2,
        numberOfColumns: 2,
        ...
        setup: (picker) => {
            picker.on('selected', (start, end) => {
                filterStartTPTDate = start.format('YYYY-MM-DD');
                filterEndTPTDate = end.format('YYYY-MM-DD');
                $('#dateRangeTPT').text(`${start.format('MMMM D, YYYY')} - ${end.format('MMMM D, YYYY')}`);
                loadData();
            });
            ...
            picker.on('clear', () => {
                filterStartTPTDate = "";
                filterEndTPTDate = "";
                $('#dateRangeTPT').text("Select Date Range");
                loadData();
            });
        },
        ranges: {
    ...
    $('#customDateTriggerTPT').on('click', function () {
        picker.show();
    });
...
function OnTabGridLoad(response) {
    ...
    if (response.length > 0) {
        $.each(response, function (index, item) {
            ...
            tabledata.push({
                ...
                Project_Det: item.project_Det,
                Product_Det: item.product_Det,
                Wipro_Product_Code: item.wipro_Product_Code,
                Sample_Qty: item.sample_Qty,
                Test_Detail: item.test_Detail,
                Vendor: item.vendor,
                Lab: item.lab,
                ...
                Testing_Charge_offer: item.testing_Charge_offer,
                Final_Testing_Charge: item.final_Testing_Charge,
                Report: item.report,
    ...
    table = new Tabulator("#thirdPartyTest_Table", {
        data: tabledata,
    ...
    table.on("cellEdited", function (cell) {
        ...
        InsertUpdateThirdParTest(cell.getRow().getData());
    });
...
$('#thirdPartyTest_Table').on('change', '.bis-upload', function () {
    const input = this;
    ...
    $.ajax({
        url: "/ThirdPartyTest/UploadTPTAttachment",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false
...
function InsertUpdateThirdParTest(rowData) {
    debugger
    if (!rowData) {
